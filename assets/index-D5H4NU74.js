...
<main id="main">
      ...
      <div class="hero-stage">
        ...
        <div class="phone-scene" role="img" aria-label="Dreidimensionale Vorschau des iPhones mit der Lernspur-App. Das Telefon dreht sich beim Scrollen.">
          <div class="phone-model">
            ...
            <div class="phone-back"><div class="camera-bump">${A}<div class="camera-face">${M}<span class="camera-flash"></span><span class="camera-sensor"></span><span class="camera-mic"></span></div></div><img src="./logo.svg" alt="" /><p>Deine Gedanken. <br>Bleiben deine.</p><small>L E R N S P U R</small></div>
  ...
  <section class="statement" aria-label="Die Idee">
    ...
    <div class="statement-footer"><span>Im Betrieb. In der Schule. Im ÜK.</span><span>Ein Ort für deinen Lernalltag ${s}</span></div>
  </section>
  <section class="story" id="entdecken" aria-labelledby="story-title">
    <div class="story-pin">
      <div class="story-heading"><div><p class="eyebrow">KLEINE MOMENTE. GROSSER UNTERSCHIED.</p><h2 id="story-title">So wird daraus <em>dein Weg.</em></h2></div><div class="story-controls"><span class="story-count" aria-live="polite">01 / 04</span><button class="rail-prev" aria-label="Vorherige Funktion">${s}</button><button class="rail-next" aria-label="Nächste Funktion">${s}</button></div></div>
      <div class="story-viewport"><div class="story-track">${R.map(e=>`
      ...
      <p class="swipe-hint">Seitlich wischen oder mit den Pfeilen entdecken ${s}</p>
    </div>
  </section>
  <section class="learning" id="dein-weg">
    <div class="learning-visual"><div class="learning-paper"><span class="paper-caption">MEIN NÄCHSTER SCHRITT</span><h3>Heute noch unsicher. <br><em>Morgen ein Aha.</em></h3><div class="paper-rule"></div><span class="paper-check">✓</span><p>Meinen eigenen Lösungsweg erklären.</p><span class="paper-doodle" aria-hidden="true">${d}</span></div><div class="module-chip">Betrieb <span>↗</span> Schule <span>↗</span> ÜK</div></div>
    <div class="learning-copy"><p class="eyebrow">DEIN ALLTAG IST DER LEHRPLAN</p><h2>Passt zu dir. <br><em>Wächst mit dir.</em></h2><p>Lehre, Gymnasium oder Praktikum: Lernspur beginnt mit deinem Weg. Sammle deine Module und Fächer, halte offene Fragen fest und mach sichtbar, was du schon kannst.</p><ul class="feature-lines"><li><span>01</span> Eigene Module, ÜKs und Fächer</li><li><span>02</span> Wochenrückblicke & offene Fragen</li><li><span>03</span> Schnelle Notizen vom Homescreen</li></ul><a class="text-link" href="./native.html">Mehr App-Einblicke ${s}</a></div>
  </section>
  <section class="privacy">
    <div class="privacy-symbol" aria-hidden="true"><img src="./logo.svg" alt="" />${d}</div><p class="eyebrow">DEIN JOURNAL GEHÖRT DIR</p><h2>Dein Kopf ist schon voll. <br><em>Deine App muss es nicht sein.</em></h2><p>Kein Login. Kein Feed. Kein Vergleich. <br>Deine Einträge bleiben auf deinem iPhone. <br>Auch dann, wenn du gerade offline bist.</p><div class="privacy-notes"><span>Auf deinem Gerät</span><span>Manuelles Backup</span><span>PDF zum Mitnehmen</span></div>
  </section>
  ...
  <section class="outro"><span aria-hidden="true">${d}</span><p class="eyebrow">DER NÄCHSTE AHA-MOMENT KOMMT BESTIMMT.</p><h2>Gib ihm <br><em>eine Lernspur.</em></h2>${b}<p>Fürs iPhone. Bald im App Store. <br><a href="./walkthrough.html">Bis dahin: interaktive Basis-Demo ansehen ↗</a></p></section>
</main>
<footer><a class="brand" href="#"><img src="./logo.svg" alt="" width="30" height="30" /><span>lernspur</span></a><span>Für deinen Lernweg. In deinem Tempo.</span><div><a href="./native.html">App-Einblicke</a><a href="https://github.com/platret/lernspur-preview">GitHub ${s}</a></div></footer>`;const f=document.querySelector(".hero"),$=document.querySelector(".phone-model"),t=document.querySelector(".story"),k=document.querySelector(".story-track"),l=document.querySelector(".story-viewport"),i=Array.from(document.querySelectorAll(".story-card")),N=document.querySelector(".story-count"),T=document.querySelector(".story-progress span"),E=document.querySelector(".rail-prev"),y=document.querySelector(".rail-next"),h=window.matchMedia("(prefers-reduced-motion: reduce)"),w=window.matchMedia("(max-width: 760px)");let a=0,g=!1,r=0;const m=e=>Math.max(0,Math.min(1,e)),p=()=>w.matches||h.matches;function x(){g=!1;const e=m(-f.getBoundingClientRect().top/Math.max(1,f.offsetHeight-window.innerHeight));$.style.transform=h.matches?"rotateX(3deg) rotateY(-16deg) rotateZ(5deg)":`translateY(${-e*18}px) rotateX(${8-e*14}deg) rotateY(${-24+e*360}deg) rotateZ(${8-e*16}deg)`;const n=p()?m(l.scrollLeft/Math.max(1,r)):m(-t.getBoundingClientRect().top/Math.max(1,t.offsetHeight-window.innerHeight));k.style.transform=p()?"":`translate3d(${-n*r}px,0,0)`,a=Math.round(n*(i.length-1)),N.textContent=`${String(a+1).padStart(2,"0")} / 04`,T.style.transform=`scaleX(${(a+1)/i.length})`,E.disabled=a===0,y.disabled=a===i.length-1}function v(){g||(g=!0,requestAnimationFrame(x))}function o(){r=Math.max(0,k.scrollWidth-l.clientWidth),t.style.height=p()?"":`${window.innerHeight+r}px`,v()}function S(e){const u=Math.max(0,Math.min(i.length-1,e))/(i.length-1),c=h.matches?"instant":"smooth";p()?l.scrollTo({left:u*r,behavior:c}):window.scrollTo({top:window.scrollY+t.getBoundingClientRect().top+u*(t.offsetHeight-window.innerHeight),behavior:c})}E.addEventListener("click",()=>S(a-1));y.addEventListener("click",()=>S(a+1));l.addEventListener("scroll",v,{passive:!0});window.addEventListener("scroll",v,{passive:!0});window.addEventListener("resize",o);h.addEventListener("change",o);w.addEventListener("change",o);new ResizeObserver(o).observe(l);o();
