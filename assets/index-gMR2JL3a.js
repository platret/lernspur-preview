...
<main id="main">
  <section class="hero" aria-labelledby="hero-title">
    <div class="hero-pin">
      <div class="hero-copy">
        ...
        <a class="text-link" href="#entdecken">Entdecke Lernspur ${t}</a>
      </div>
        ...
        <div class="phone-scene" role="img" aria-label="Dreidimensionale Vorschau des iPhones mit der Lernspur-App. Das Telefon dreht sich beim Scrollen.">
          <div class="phone-model">
            ...
            <div class="phone-back">
              ...
              <div class="camera-bump">${v(246,133,26)}<div class="camera-face">${T}<span class="camera-flash"></span><span class="camera-sensor"></span><span class="camera-mic"></span></div></div>
            </div>
  ...
  <section class="statement" aria-label="Die Idee">
    ...
    <div class="statement-footer"><span>Im Betrieb. In der Schule. Im ÜK.</span><span>Ein Ort für deinen Lernalltag ${t}</span></div>
  </section>
  <section class="story" id="entdecken" aria-labelledby="story-title">
    <div class="story-pin">
      <div class="story-heading"><div><p class="eyebrow">KLEINE MOMENTE. GROSSER UNTERSCHIED.</p><h2 id="story-title">So wird daraus <em>dein Weg.</em></h2></div><div class="story-controls"><span class="story-count" aria-live="polite">01 / 04</span><button class="rail-prev" aria-label="Vorherige Funktion">${t}</button><button class="rail-next" aria-label="Nächste Funktion">${t}</button></div></div>
      <div class="story-viewport"><div class="story-track">${P.map(e=>`
      ...
      <p class="swipe-hint">Seitlich wischen oder mit den Pfeilen entdecken ${t}</p>
    </div>
  </section>
  ...
  <section class="plans" id="plus">
    ...
    <div class="plan-grid">
      <article class="plan plan-free"><div class="plan-label">LERNSPUR FREE</div><h3>Dein Alltag. <br>Gut aufgehoben.</h3><p class="price">CHF 0 <span>/ dauerhaft</span></p><ul><li>Notizen, Fotos & Sprachmemos</li><li>Module, Wochenziele & Rückblicke</li><li>Lernberichte mit PDF-Export</li><li>Homescreen-Widget & lokales Backup</li></ul><a class="button" href="./release.html">Version 1.0 ansehen ${t}</a></article>
      <article class="plan plan-plus"><div class="plan-label">LERNSPUR PLUS <span>MONATSABO</span></div><h3>Ein bisschen mehr. <br>Für dein Weiterkommen.</h3><p class="price">CHF 10 <span>/ Monat · geplant</span></p><ul><li>Eigene Lernkarten pro Modul</li><li>Üben, aufdecken & Lernstand behalten</li><li>Offene Karten zuerst wiederholen</li><li>Aufdecken, wischen & weiterlernen</li></ul><a class="button button-light" href="./release.html">Plus in Version 1.0 ${t}</a><p class="plan-note">Optionales Monatsabo über den App Store. Der finale Preis erscheint vor dem Kauf. Keine Cloud-Synchronisation oder KI enthalten. Veröffentlichung in Vorbereitung.</p></article>
    </div>
  </section>
  <section class="faq" id="fragen"><div><p class="eyebrow">NOCH KURZ GEFRAGT</p><h2>Gut zu <em>wissen.</em></h2></div><div class="faq-list">
    <details><summary>Kann ich Lernspur schon ausprobieren?<span>+</span></summary><p>Version 1.0 wird gerade für den App Store vorbereitet. Die Release-Vorschau zeigt die echte App mit Notizen, Wochenzielen, Lernberichten und Lernkarten. Sobald Apple die Veröffentlichung freigibt, findest du hier den direkten Download.</p><a href="./release.html">Version 1.0 ansehen ↗</a></details>
    <details><summary>Bleiben meine Einträge wirklich lokal?<span>+</span></summary><p>Ja. Die native App speichert auf deinem Gerät. Du kannst deinen Namen manuell eingeben oder optional Apple verwenden. Es gibt kein Lernspur-Serverkonto, keine Cloud-Synchronisation und keine KI-Verarbeitung. Erstelle ein manuelles Backup, bevor du die App löschst.</p></details>
    <details><summary>Was bekomme ich mit Plus?<span>+</span></summary><p>Plus bietet eigene Lernkarten, Zuordnung zu Modulen und einen Übungsmodus mit Wischgesten. Dein Lernstand bleibt lokal gespeichert. Das optionale Monatsabo wird über Apple gekauft und verwaltet. Cloud-Synchronisation und KI sind nicht enthalten.</p></details>
  ...
  <section class="outro"><span aria-hidden="true">${g}</span><p class="eyebrow">DER NÄCHSTE AHA-MOMENT KOMMT BESTIMMT.</p><h2>Gib ihm <br><em>eine Lernspur.</em></h2>${L}<p>Fürs iPhone. Bald im App Store. <br><a href="./release.html">Bis dahin: Version 1.0 entdecken ↗</a></p></section>
</main>
<footer><a class="brand" href="#"><img src="./logo.svg" alt="" width="30" height="30" /><span>lernspur</span></a><span>Für deinen Lernweg. In deinem Tempo.</span><div><a href="./native.html">App-Einblicke</a><a href="./privacy.html">Datenschutz</a><a href="./support.html">Hilfe</a><a href="https://github.com/platret/lernspur-preview">GitHub ${t}</a></div></footer>`;const M=document.querySelector(".hero"),I=document.querySelector(".phone-model"),p=document.querySelector(".story"),A=document.querySelector(".story-track"),u=document.querySelector(".story-viewport"),d=Array.from(document.querySelectorAll(".story-card")),K=document.querySelector(".story-count"),C=document.querySelector(".story-progress span"),$=document.querySelector(".rail-prev"),D=document.querySelector(".rail-next"),f=window.matchMedia("(prefers-reduced-motion: reduce)"),R=window.matchMedia("(max-width: 760px)");let r=0,w=!1,h=0;const k=e=>Math.max(0,Math.min(1,e)),b=()=>R.matches||f.matches;function B(){w=!1;const e=k(-M.getBoundingClientRect().top/Math.max(1,M.offsetHeight-window.innerHeight));I.style.transform=f.matches?"rotateX(3deg) rotateY(-16deg) rotateZ(5deg)":`translateY(${-e*18}px) rotateX(${8-e*14}deg) rotateY(${-24+e*360}deg) rotateZ(${8-e*16}deg)`;const s=b()?k(u.scrollLeft/Math.max(1,h)):k(-p.getBoundingClientRect().top/Math.max(1,p.offsetHeight-window.innerHeight));A.style.transform=b()?"":`translate3d(${-s*h}px,0,0)`,r=Math.round(s*(d.length-1)),K.textContent=`${String(r+1).padStart(2,"0")} / 04`,C.style.transform=`scaleX(${(r+1)/d.length})`,$.disabled=r===0,D.disabled=r===d.length-1}function y(){w||(w=!0,requestAnimationFrame(B))}function m(){h=Math.max(0,A.scrollWidth-u.clientWidth),p.style.height=b()?"":`${window.innerHeight+h}px`,y()}function N(e){const n=Math.max(0,Math.min(d.length-1,e))/(d.length-1),a=f.matches?"instant":"smooth";b()?u.scrollTo({left:n*h,behavior:a}):window.scrollTo({top:window.scrollY+p.getBoundingClientRect().top+n*(p.offsetHeight-window.innerHeight),behavior:a})}$.addEventListener("click",()=>N(r-1));D.addEventListener("click",()=>N(r+1));u.addEventListener("scroll",y,{passive:!0});window.addEventListener("scroll",y,{passive:!0});window.addEventListener("resize",m);f.addEventListener("change",m);R.addEventListener("change",m);new ResizeObserver(m).observe(u);m();
